...
export default async function ShoppingPage() {
  ...
  const { data: items } = await supabase
    ...
    .select("id, text, quantity, aisle, completed_at, source_recipe_id")
  ...
  // Group todo by aisle.
  const byAisle = new Map<string, typeof todo>();
  for (const i of todo) {
    const key = i.aisle?.trim() || "Other";
    if (!byAisle.has(key)) byAisle.set(key, []);
    byAisle.get(key)!.push(i);
  }
  ...
  return (
    <>
      ...
      <div className="flex-1 space-y-4 px-4 py-4 pb-8">
        <form action={addShoppingItem} className="grid grid-cols-[1fr_5rem_5rem_auto] gap-2">
          <div>
          ...
          <div>
            <Label htmlFor="aisle" className="sr-only">
              Aisle
            </Label>
            <Input id="aisle" name="aisle" placeholder="aisle" autoComplete="off" />
          </div>
        ...
        {Array.from(byAisle.entries()).map(([aisle, list]) => (
          <section key={aisle} className="space-y-1">
